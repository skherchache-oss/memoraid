...
const PremiumStore: React.FC<PremiumStoreProps> = ({ onUnlockPack, unlockedPackIds }) => {
    // Fix: Destructured "language" from useLanguage to resolve "Language" not found error
                ...
                <div className="relative z-10 max-w-3xl mx-auto">
                    ...
                    >
                        ...
                        <div className="text-left">
                            <p className="text-[10px] font-black uppercase tracking-widest text-slate-400">{t('premium_new_packs_label')}</p>
                            <p className="text-sm font-bold text-white">{t('premium_new_packs_desc')}</p>
                        </div>
            ...
            <div className="container mx-auto px-4 -mt-10 relative z-20">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8 max-w-4xl mx-auto">
                        ...
                        <div className="bg-white dark:bg-zinc-800 p-8 rounded-[40px] shadow-xl border border-slate-100 dark:border-zinc-700 flex flex-col group">
                            <div className="text-center mb-6">
                                <span className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 mb-2 block">{t('premium_monthly')}</span>
                            ...
                            <div className="flex-grow">
                                <ul className="space-y-4 mb-8">
                                    <li className="flex items-start gap-3 text-sm text-slate-600 dark:text-zinc-300 font-medium">
                                        ...
                                        <span>{parseContent(t('premium_limitless_creation'))}</span>
                                    </li>
                                    <li className="flex items-start gap-3 text-sm text-slate-600 dark:text-zinc-300 font-medium">
                                        <CheckCircleIcon className="w-5 h-5 text-emerald-500 flex-shrink-0 mt-0.5" />
                                        <span>{t('premium_full_features')}</span>
                                    </li>
                                    <li className="flex items-start gap-3 text-sm text-slate-600 dark:text-zinc-300 font-medium">
                                        <CheckCircleIcon className="w-5 h-5 text-emerald-500 flex-shrink-0 mt-0.5" />
                                        <span>{t('premium_image_gen')}</span>
                                    </li>
                                    <li className="flex items-start gap-3 text-sm text-slate-600 dark:text-zinc-300 font-medium">
                                        <CheckCircleIcon className="w-5 h-5 text-emerald-500 flex-shrink-0 mt-0.5" />
                                        <span>{t('premium_no_ads')}</span>
                                    </li>
                                </ul>
                            </div>
                        ...
                        <div className="bg-white dark:bg-zinc-800 p-8 rounded-[40px] shadow-2xl border-2 border-emerald-500 flex flex-col relative group">
                            <div className="absolute -top-4 left-1/2 -translate-x-1/2 bg-emerald-500 text-white px-6 py-1 rounded-full text-[10px] font-black uppercase tracking-widest shadow-lg shadow-emerald-200/30">
                                {t('premium_best_value')}
                            </div>
                            
                            <div className="text-center mb-6">
                                <span className="text-[10px] font-black uppercase tracking-[0.2em] text-emerald-600 dark:text-emerald-400 mb-2 block">{t('premium_annual')}</span>
                                <div className="flex items-baseline justify-center gap-1">
                ...
                <div className="max-w-4xl mx-auto mb-20">
                    <div className="bg-slate-50 dark:bg-zinc-800/50 rounded-3xl p-6 md:p-8 border border-slate-200 dark:border-zinc-700">
                        <h3 className="flex items-center gap-2 text-xs font-black uppercase tracking-widest text-slate-500 mb-4">
                            <InfoIcon className="w-4 h-4" /> {t('premium_important_notes')}
                        </h3>
                ...
                <div id="knowledge-packs-section" className="mb-20 scroll-mt-24">
                    <div className="flex items-center gap-4 mb-12 justify-center">
                        ...
                        <h2 className="text-sm font-black uppercase tracking-[0.3em] text-slate-400 text-center">{t('premium_packs_title')}</h2>
                        <div className="h-px bg-slate-200 dark:bg-zinc-800 flex-grow max-w-[100px]"></div>
                    </div>

                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 max-w-6xl mx-auto">
                        {/* Pack: Apprendre à apprendre */}
                        <div className={`group bg-white dark:bg-zinc-800 rounded-[40px] shadow-xl border overflow-hidden hover:-translate-y-2 transition-all duration-500 ${isApprendreUnlocked ? 'border-emerald-500 ring-2 ring-emerald-500/20' : 'border-slate-100 dark:border-zinc-700'}`}>
                            <div className="h-48 bg-gradient-to-br from-indigo-600 to-blue-700 p-8 flex flex-col justify-between relative overflow-hidden">
                                ...
                                <div className="bg-white/20 backdrop-blur-md w-fit px-3 py-1 rounded-full text-[10px] font-black text-white uppercase tracking-widest border border-white/20">
                                    {isApprendreUnlocked ? t('premium_unlocked') : t('premium_one_time_purchase')}
                                </div>
                            ...
                            <div className="p-8">
                                ...
                                <div className="flex items-center justify-between mb-8">
                                    <div className="flex flex-col">
                                        <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">{t('premium_fixed_price')}</span>
                                        <span className="text-2xl font-black text-slate-900 dark:text-white">3,99 €</span>
                                    </div>
                                    <div className="text-right">
                                        <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">{t('premium_content_label')}</span>
                                        <span className="block text-sm font-bold text-slate-700 dark:text-zinc-300">{APPRENDRE_PACK.capsuleCount} {t('modules')}</span>
                                    </div>
                                </div>
                                <button 
                                ...
                                >
                                    {isApprendreUnlocked ? t('premium_access_content_btn') : t('premium_unlock_pack_btn')}
                                </button>
                            </div>
                        </div>
                        ...
                        <div className="group bg-white dark:bg-zinc-800 rounded-[40px] shadow-xl border border-slate-100 dark:border-zinc-700 overflow-hidden opacity-60 grayscale hover:grayscale-0 transition-all duration-500">
                            <div className="h-48 bg-gradient-to-br from-slate-600 to-slate-700 p-8 flex flex-col justify-between">
                                <div className="bg-white/20 backdrop-blur-md w-fit px-3 py-1 rounded-full text-[10px] font-black text-white uppercase tracking-widest border border-white/20">
                                    {/* Fix: use language instead of Language to resolve undefined error */}
                                    {language === 'fr' ? 'Langues' : 'Languages'}
                                </div>
                                {/* Fix: use language instead of Language to resolve undefined error */}
                                <h3 className="text-2xl font-black text-white leading-tight">{language === 'fr' ? 'Pack Anglais Business' : 'Business English Pack'}</h3>
                            </div>
                            <div className="p-8">
                                <p className="text-slate-500 dark:text-zinc-400 text-sm mb-6 leading-relaxed">
                                    {/* Fix: use language instead of Language to resolve undefined error */}
                                    {language === 'fr' ? 'Maîtrisez le vocabulaire professionnel et les structures clés pour vos réunions.' : 'Master professional vocabulary and key structures for your meetings.'}
                                </p>
                                <div className="flex items-center justify-between mb-8">
                                    <span className="text-2xl font-black text-slate-900 dark:text-white">9,99 €</span>
                                    <span className="text-sm font-bold text-slate-400 uppercase tracking-widest italic">{t('premium_soon')}</span>
                                </div>
                ...
                <div className="max-w-2xl mx-auto p-8 border-2 border-dashed border-slate-200 dark:border-zinc-800 rounded-[40px] text-center mb-10">
                    <h3 className="text-xs font-black text-slate-400 uppercase tracking-[0.2em] mb-4">{t('premium_transparency_title')}</h3>
                    <p className="text-sm text-slate-500 dark:text-zinc-500 leading-relaxed font-medium">
                        {t('premium_transparency_desc')}
                    </p>
                </div>
            </div>
        </div>
