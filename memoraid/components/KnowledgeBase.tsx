        ...
        <div className="w-full min-h-screen animate-fade-in pb-32">
            <header className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-12">
                <div className="flex items-center">
                    ...
                    <div>
                         <h2 className="text-2xl md:text-3xl font-black text-slate-900 dark:text-white tracking-tight">{t('my_knowledge_base')}</h2>
                         <p className="text-[10px] font-black uppercase tracking-widest text-slate-400 mt-1">{t('knowledge_and_revisions')}</p>
                    </div>
                </div>
            ...
            {classCapsules.length > 0 && (
                <section className="mb-12">
                    <div className="flex items-center gap-3 mb-6">
                        ...
                        <h3 className="text-sm font-black uppercase tracking-widest text-slate-500">{t('class_modules')}</h3>
                    </div>
            ...
            <section className="mb-20">
                <div className="flex items-center gap-3 mb-6">
                    ...
                    <h3 className="text-sm font-black uppercase tracking-widest text-slate-500">{t('personal_modules')}</h3>
                </div>
                {personalCapsules.length > 0 ? renderCapsuleGrid(personalCapsules) : (
                    <div className="text-center py-16 bg-slate-50/50 dark:bg-zinc-900/30 rounded-[40px] border-2 border-dashed border-slate-200 dark:border-zinc-800">
                        ...
                        <p className="text-slate-400 font-bold mb-6">{t('empty_library_msg')}</p>
                        <button onClick={onNewCapsule} className="px-8 py-3 bg-emerald-600 text-white rounded-2xl font-black uppercase text-xs tracking-widest hover:bg-emerald-700 transition-all">{t('create_first_module')}</button>
                    </div>
                )}
            </section>
